...
export function findVerbs(words) {
  ...
  // Your code here
  //const verbs = words.map((word) => word.slice(0, 2) === "to");
  return words.filter(
    (word) => word.startsWith("to ") && !word.includes(" to ")
  );
}
...
export function findSentencesContaining(sentences, str) {
  ...
  //const eachSentences = sentences.match(/(?<=[.!?])\s+/);
  return sentences.filter((sentence) =>
    sentence.toLowerCase().includes(lowercaseString)
  );
}
